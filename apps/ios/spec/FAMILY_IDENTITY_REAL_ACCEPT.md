...
部署走查不会退出或解散账号里已有的未知家庭；发现已有家庭即停止。双方成员查询必须是 200 且 `family: null`，否则立即停止。本轮家庭清理失败则走查失败，并报告遗留家庭需人工处理。`public_https_service` 只接受公网 `https://`，本机与私网 HTTP/HTTPS 不能让该项通过。
