...
| 格式 | 图片 `image/jpeg`、`image/png`；声音 `audio/mp4`（含 m4a/aac）。只认声明的 MIME **且** 核文件头 | 与个人库已有照片/录音一致。仅 Content-Type 不够。F2 **不解码**整文件：JPEG 看 SOI（`FF D8 FF`），PNG 看 8 字节签名，音频看 `ftyp`。带正确文件头的截断体仍可能被收下。 |
...
| 幂等 | `Idempotency-Key` 可选。同一 `userId + uploadMedia + key` 且内容指纹相同则复用同一对象，**但复用前必须能读到与 `contentSha256` 一致的文件**。文件缺失或哈希不对时，用这次传入的字节写回原 `storageKey` 并再核验；写不回或核验失败则不得返回已保存。指纹不同 `CONFLICT`。同一用户同一内容哈希即使换 key 也走同一套核验/修复 | 重试不复制，也不把不可读对象当成已保存。 |
| 校验失败 | 空体、声明 MIME 不支持、或文件头与声明不符 → `MEDIA_UNSUPPORTED` 或 `MEDIA_CORRUPT`。不写对象。F2 不保证拒绝所有截断文件 | 只挡住明显错类型/空体，不冒充完整解码。 |
...
| `MEDIA_NOT_FOUND` | 404 | 无此对象；或元数据在但文件缺失、长度不对、或内容哈希与 `contentSha256` 不符（不冒充可读） |
| `MEDIA_TOO_LARGE` | 413 | 超过 8 MiB |
| `MEDIA_UNSUPPORTED` | 415 | MIME 不支持 |
| `MEDIA_CORRUPT` | 400 | 空体，或文件头与声明类型不符。**不是**完整结构/截断检测 |
