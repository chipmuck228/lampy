...
describe('secure family session store', () => {
  ...
  it('leaves the live session in place when writing the pending revoke key fails', async () => {
    const memory = new Map<string, string>();
    const kv = {
      async getItem(key: string) {
        return memory.get(key) ?? null;
      ...
      async setItem(key: string, value: string) {
        if (key === FAMILY_PENDING_REVOKE_SECURE_KEY) {
    ...
    expect(await store.getSessionToken()).toBe('ses_live');
    expect(await store.getUserId()).toBe('usr_alice');
    expect(await store.getPendingRevoke()).toBeNull();
    expect(memory.has(FAMILY_SESSION_SECURE_KEY)).toBe(true);
    expect(memory.has(FAMILY_PENDING_REVOKE_SECURE_KEY)).toBe(false);
  });
});
