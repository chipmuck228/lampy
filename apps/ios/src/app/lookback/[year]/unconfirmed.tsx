...
export default function LookbackYearUnconfirmedScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      let cancelled = false;
      getUseCases()
        .then((app) => app.getHistoryYearUnconfirmed(year, offset))
        .then((next) => {
          if (!cancelled) {
            setView((current) => {
  ...
  return (
    <LookbackScaffold title={ready?.title || `${year}年，月份未确认`} path={`/lookback/${year}/unconfirmed`}>
      ...
      {ready?.hasMore ? (
        <Pressable
          ...
          testID="lookback-year-unconfirmed-more"
