...
仅作者。服务端把该分享标为 `revoked` 并持久。之后列表、单条快照和分享媒体对任何成员都不再提供（404）。重复撤回幂等。他人撤回 403。退出/被移除/解散后读取仍是 404。客户端成功撤回后清该条家庭缓存；其他设备靠下一次授权列表核验隔离。请求失败保持可重试，不显示「已撤回」。资格已失去时先隐藏再清该账号该家缓存；清理失败仍隐藏。不删个人 Moment/照片/录音，不删 F2 对象（可能被其他 active 分享引用）。无法收回已导出副本。
...
家庭时间线、Transmission 改写、跨设备同步、媒体自动上传、创建者移交、删除账号、F2 垃圾回收、微信领域修改。本目录已含 F2 媒体、F3 快照、F4 授权读取/家庭缓存与 F5 撤回/资格变化清理，不是家人已收到，也不是身份闭环验收通过。`identityLoopAccepted` 仍为 false，家庭入口仍关闭。撤回后无法收回用户已导出到 App 外的副本。
